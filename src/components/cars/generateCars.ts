import { garageArray } from "../garage/garage";
...
function arrayRandElement(brand: String[], model: String[]) {
    let randBrand = Math.floor(Math.random() * brand.length);
    let randModel= Math.floor(Math.random() * model.length);
    return `${brand[randBrand]} ${model[randModel]}`;
}
